...
- 「解析」(ページ説明・あらすじ・タグ)と、その設定・既定プロンプト(`DEFAULT_SYSTEM_PROMPT`)は
  まだ漫画向け。M4(章の構造化と要約)で本向けに置き換える。
- ユーザーの設定では「解析」のシステムプロンプトが独自の漫画向けの文になっている(自動では置き換えない)。
- ルビ混入の修正より前に文字起こししたページは、やり直すと直る。
